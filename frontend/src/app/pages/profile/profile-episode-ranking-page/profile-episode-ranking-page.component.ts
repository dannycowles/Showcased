...
@Component({
  ...
  imports: [RankedEpisodeListFullComponent, RouterLink],
  standalone: true,
})
export class ProfileEpisodeRankingPageComponent implements OnInit {
  ...
  async openEpisodeSelectModal() {
    try {
      ...
      const episodeModalRef = this.modalService.open(
        EpisodeSelectModalComponent,
        {
      ...
      episodeModalRef.componentInstance.selectedShowTitle =
        this.selectedShow.title;
      episodeModalRef.componentInstance.selectedSeason = this.selectedSeason;
      episodeModalRef.componentInstance.onAddEpisode = (
        episode: EpisodeRankingData,
      ) => {
  ...
  async removeEpisodeFromRankingList(removeId: number) {
    try {
      ...
      this.rankingEntries = this.rankingEntries.filter(
        (episode) => episode.episodeId !== removeId,
      );
    } catch (error) {
      console.error(error);
    }
  ...
  async updateEpisodeRankingList() {
    try {
      const updates: UpdateEpisodeRankingDto[] = this.rankingEntries.map(
        (episode) => ({
